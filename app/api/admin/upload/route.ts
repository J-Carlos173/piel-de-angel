...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[admin/upload] Medusa error:", await res.text());
      return NextResponse.json({ error: "Error al subir imagen" }, { status: 400 });
    }

    const data = await res.json();
    let url: string = data.files?.[0]?.url ?? data.uploads?.[0]?.url ?? "";

    // Medusa devuelve localhost:9000 internamente — reemplazar con URL pública de Railway
    if (url && BASE) {
